...
const ProjectsPage = () => {
  const handleRegisterc = e => {
    ...
    Axios.get("/auth/status").then(res => {
      if (res.data.loggedin) window.location.href = "https://imjo.in/uaXZcM";
      else {
        Swal.fire({
          icon: "info",
          ...
          if (res.value) window.location.href = "http://tirutsava.com/login";
        });
      }
    });
  };
  ...
  const handleRegisterr = e => {
    e.preventDefault();
    Axios.get("/auth/status").then(res => {
      if (res.data.loggedin) window.location.href = "https://imjo.in/5nYqca";
      else {
        Swal.fire({
          icon: "info",
          title: "Registration Failed!!",
  ...
  const handleRegisterg = e => {
    e.preventDefault();
    Axios.get("/auth/status").then(res => {
      if (res.data.loggedin) window.location.href = "https://imjo.in/NXv47Y";
      else {
        Swal.fire({
          icon: "info",
          title: "Registration Failed!!",
        ...
        <section className="my-5 adjust-margin adjust-margin-bottom">
          ...
          <MDBRow>
            ...
            >
              ...
              <div className="buttons">
                {" "}
                <button className="bt-hover color-7" onClick={handleRegisterc}>
                  Register
                </button>
              </div>
            </MDBCol>
